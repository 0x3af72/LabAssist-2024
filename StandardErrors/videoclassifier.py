...
class VideoClassifier(LightningModule):
    def __init__(self, learning_rate=1e-4, batch_size=6, num_worker=0):
        ...
        # weights
        class_weights = torch.tensor([1.0, 1.0, 1.0]) # 2 2 0.3
    ...
    def validation_step(self, batch, batch_idx):
        ...
        metric = self.metric(pred, label.to(torch.int64))
        # print(loss)
        self.validation_step_loss.append(loss.detach())
        self.validation_step_metric.append(metric.detach())
        # print(self.validation_step_loss)
        return {'val_loss': loss, 'val_metric': metric.detach()}
    
    def on_validation_epoch_end(self):
        
        if len(self.validation_step_loss) == 0:
            avg_loss = 0.0
            avg_metric = 0.0
        else:
    ...
    def on_test_epoch_end(self):
        ...
        if len(self.test_step_label) == 0 or len(self.test_step_pred) == 0:
